...
export const olayBildirimleri = pgTable('olay_bildirimleri', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  isletmeId: uuid('isletme_id').notNull(),
  subeId: uuid('sube_id'),
  takipKodu: text('takip_kodu').notNull(),
  tur: text('tur').notNull(),
  siddet: text('siddet').notNull(),
  olayZamani: timestamp('olay_zamani', { withTimezone: true }).notNull(),
  yer: text('yer').notNull().default(''),
  aciklama: text('aciklama').notNull(),
  ilkMudahale: text('ilk_mudahale').notNull().default(''),
  kisiId: uuid('kisi_id'),
  isimsiz: boolean('isimsiz').notNull(),
  bildirenId: uuid('bildiren_id'),
  bildirenSifreli: text('bildiren_sifreli'),
  durum: text('durum').notNull().default('yeni'),
  sorumluId: uuid('sorumlu_id'),
  kokNeden: text('kok_neden'),
  alinanOnlem: text('alinan_onlem'),
  resmiBildirim: text('resmi_bildirim'),
  kapatanId: uuid('kapatan_id'),
  kapanisZamani: timestamp('kapanis_zamani', { withTimezone: true }),
  zaman: timestamp('zaman', { withTimezone: true }).notNull().defaultNow(),
});
...
export const sikayetler = pgTable('sikayetler', {
  ...
  kanal: text('kanal').notNull(),
  kategori: text('kategori').notNull(),
  resmiNo: text('resmi_no').notNull().default(''),
  kisiId: uuid('kisi_id'),
  basvuran: text('basvuran').notNull(),
  iletisim: text('iletisim').notNull().default(''),
  konu: text('konu').notNull(),
  aciklama: text('aciklama').notNull(),
  alinisGunu: date('alinis_gunu', { mode: 'string' }).notNull(),
  sonTarih: date('son_tarih', { mode: 'string' }).notNull(),
  durum: text('durum').notNull().default('acik'),
  sorumluId: uuid('sorumlu_id'),
  cevap: text('cevap'),
  cevaplayanId: uuid('cevaplayan_id'),
  cevapZamani: timestamp('cevap_zamani', { withTimezone: true }),
  kaydedenId: uuid('kaydeden_id').notNull(),
  zaman: timestamp('zaman', { withTimezone: true }).notNull().defaultNow(),
});

export const dofler = pgTable('dofler', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  isletmeId: uuid('isletme_id').notNull(),
  kaynak: text('kaynak').notNull(),
  olayId: uuid('olay_id'),
  sikayetId: uuid('sikayet_id'),
  baslik: text('baslik').notNull(),
  kokNeden: text('kok_neden').notNull().default(''),
  aksiyon: text('aksiyon').notNull(),
  sorumluId: uuid('sorumlu_id').notNull(),
  termin: date('termin', { mode: 'string' }).notNull(),
  durum: text('durum').notNull().default('acik'),
  tamamlamaNotu: text('tamamlama_notu'),
  tamamlayanId: uuid('tamamlayan_id'),
  tamamlamaZamani: timestamp('tamamlama_zamani', { withTimezone: true }),
  dogrulayanId: uuid('dogrulayan_id'),
  dogrulamaNotu: text('dogrulama_notu'),
  dogrulamaZamani: timestamp('dogrulama_zamani', { withTimezone: true }),
  acanId: uuid('acan_id').notNull(),
  zaman: timestamp('zaman', { withTimezone: true }).notNull().defaultNow(),
});
